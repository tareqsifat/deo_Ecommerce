...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <h5 class="mb-0 h6">{{translate('Area Cost Edit')}}</h5>
</div>
  	    ...
  	    <!--        @endforeach-->
              ...
              <form class="p-4" action="{{ route('cities.update', $city->id) }}" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                      <label for="country">{{translate('Area Zone')}}</label>
                      <select class="select2 form-control aiz-selectpicker" name="area_id" data-toggle="select2" data-placeholder="Choose ..." data-live-search="true">
                          @foreach ($areaZones as $area)
                              <option value="{{ $area->id }}" @if($area->id == $city->area_id) selected @endif>{{ $area->area }}</option>
                          @endforeach
                      </select>
                  </div>
