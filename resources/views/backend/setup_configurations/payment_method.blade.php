    ...
    <div class="row">
        ...
        @php
            $areas = App\Models\AreaZone::get();
        @endphp
        <div class="col-sm-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6 ">{{translate("Zone Wise Payment Method")}}</h5>
                </div>
                <div class="card-body">
                    <div class="form-group row" id="select">
                        <div class="col-md-6">
                            <label class="aiz-switch aiz-switch-success mb-0 h5">{{ translate("Select Zone") }}</label>
                        </div>
                        <div class="col-md-6">
                            <select id="zone_payment" name="payment_method" class="form-control" required>
                    ...
                    <div class="form-group row">
                        <div class="col-md-6">
                            <label class="aiz-switch aiz-switch-success mb-0 h6">COD status</label>
                        </div>
                        <div class="col-md-6">
                            <label class="aiz-switch aiz-switch-success mb-0">
                                 <input id="payment_discount_status" name="payment_discount_status" value="1" type="checkbox">
                                <span class="slider round"></span>
                            </label>
                        </div>
                    </div>
...
@section('script')
    <script type="text/javascript">
        $('#payment_discount_status').on('change', function(e){
            if($("#payment_discount_status").is(":checked")){
                zone_selected("1");
            }
            else{
                zone_selected(null);
        ...
        function zone_selected(cod_status){
            if($('#zone_payment').val() != ""){
                zone_payment(cod_status);
            }
            else{
                AIZ.plugins.notify('danger','please select an area from dropdown');
            }
        }
        
        function zone_payment(payment){
            data= {
                "_token": "{{ csrf_token() }}",
                id:$("#zone_payment").val(),
            ...
            $.ajax({
                ...
                method: "POST",
                url: "{{ route('zone_wise_payment_method') }}",
                data: data,
