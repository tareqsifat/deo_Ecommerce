...
<body>
	<div>
		...
		<div style="background: #eceff4;padding: 1.5rem;">
			...
			<table>
				<tr>
					<td style="font-size: 1rem;" class="strong">{{ $order->seller->name }}</td>
					@if($order->payment_status=='paid' && $order->payment_status!='cash_on_delivery')
					<td class="text-right strong" style="font-size: 1rem;">{{  translate('PAID') }}</td>
					@else
					<td class="text-right strong" style="font-size: 1rem;">{{  translate('UNPAID') }}</td>
					@endif
				</tr>
				<tr>
					<td class="gry-color small">{{ $order->seller->address }}, {{ $order->seller->thana }}, {{ $order->seller->city }}</td>
					<td class="text-right"></td>
				</tr>
				<tr>
					<td class="gry-color small">{{  translate('Phone') }}: {{$order->seller->phone }}</td>
					<td class="text-right small"><span class="gry-color small">{{  translate('Order ID') }}:</span> <span class="strong">{{ $order->code }}</span>
		...
		<div style="padding: 1.5rem;padding-bottom: 0">
            <table>
				...
				<tr><td class="gry-color small">{{ $shipping_address->address }}, {{ $shipping_address->thana }}, {{ $shipping_address->city }}</td></tr>
				...
				<tbody class="strong">
	                @foreach ($order->orderDetails as $key => $orderDetail)
		                @if ($orderDetail->product != null)
							<tr class="">
								<td>{{ $orderDetail->product->getTranslation('name') }} @if($orderDetail->variation != null) ({{ $orderDetail->variation }}) @endif
								</td>
								<td>
