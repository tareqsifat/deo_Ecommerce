...
@section('content')
    ...
    <div class="col-lg-12 home-banner-area mb-4 pt-3">
        <div class="container">
                                ...
                                <div class="col mb-2">
                                    <a href="{{ route('product', $product->slug) }}" class="d-block p-2 text-reset bg-white h-100 rounded">
                                        <div class="row gutters-5 align-items-center">
                                            ...
                                            <div class="col-xxl">
                                                <div class="fs-16">
                                                    <p class="d-block text-primary fw-600">{{ home_discounted_base_price($product) }}</p>
        ...
        <div class="container">
            <div class="px-2 py-4 px-md-4 py-md-3 shadow-sm rounded">
                ...
                <div class="d-flex flex-wrap mb-3 align-items-baseline border-bottom">
                    ...
                    <a href="{{ route('flash-deal-details', $flash_deal->slug) }}" class="ml-auto mr-0 btn btn-primary btn-sm shadow-md">{{ translate('View More') }}</a>
                </div>
                <div class="responsive_section_mobile">
                    <div class="row justify-content-center">
                        @foreach ($flash_deal->flash_deal_products->take(10) as $key => $flash_deal_product)
                            @php
                ...
                <div class="responsive_section_pc">
                    <div class="aiz-carousel gutters-10" id="flash_deal_carousel" data-items="6" data-xl-items="5" data-lg-items="4"  data-md-items="3" data-xs-items="2" data-arrows='true'>
    ...
    <section class="mb-4">
        <div class="container">
            <div class="row gutters-10">
                ...
                @endif
                <div class="mobile_footer_responsive_correction col-12" style="height: 50px;"></div>
            </div>
        </div>
    </section>
    @endif
    ...
    <script>
        $(document).ready(function(){
            if($(window).width() > 576){
                ...
                $.post('{{ route('mobile.home.section.featured') }}',{_token:'{{ csrf_token() }}'}, function(data){
                    $('#section_featured').html(data);
                });
                $.post('{{ route('mobile.home.section.best_selling') }}', {_token:'{{ csrf_token() }}'}, function(data){
                    $('#section_best_selling').html(data);
                });
                $.post('{{ route('mobile.home.section.home_categories') }}', {_token:'{{ csrf_token() }}'}, function(data){
                    $('#section_home_categories').html(data);
                });
            ...
            if($(window).width() < 576){
                ...
                $('.responsive_section_pc').hide();
                $('.header_menu_labels').hide();
            }
            else if($(window).width() > 576){
                $('.absolute-top-right').show();
                $('.responsive_section_mobile').hide();
                $('.mobile_footer_responsive_correction').hide();
            }
        });
        // function arrows(){
        //     // $('#flash_deal_carousel').attr('data-arrows',true);
        //     document.getElementById("flash_deal_carousel").setAttribute("data-arrows", "true");
        // }
    </script>
@endsection
