...
class NotificationUtility
{
    public static function sendOrderPlacedNotification($order, $request = null)
    {
        ...
        if (env('MAIL_USERNAME') != null) {
            ...
            try {
                ...
                Mail::to(User::where('user_type', 'seller')->first()->email)->queue(new InvoiceEmailManager($array));
            } catch (\Exception $e) {
    ...
    public static function sendShipmentNotification($order)
    {        
        ...
        if (env('MAIL_USERNAME') != null) {
            $array['view'] = 'emails.invoice';
            // dd($order->all());
            $array['subject'] = translate('Your order has been shipped') . ' - ' . $order->code;
            $array['from'] = env('MAIL_FROM_ADDRESS');
            $array['order'] = $order;
            try {
                Mail::to($order->user->email)->queue(new InvoiceEmailManager($array));
